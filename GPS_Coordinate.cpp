...
static Float xPi = pi * 3000 / 180.0;
...
static Float transformLat(GPS_Coordinate point);
static Float transformLng(GPS_Coordinate point);
...
public:
	...
	template<class T>
	Coord(T _lng,T _lat):lng(_lng),lat(_lat)
	{
	}
	Coord(GPS_Coordinate t):
	lng(t.longitude),lat(t.latitude)
	{
	}

	GPS_Coordinate toGpsCoordinate()
	{
		return GPS_Coordinate(lng.convert_to<double>(), lat.convert_to<double>());
	}
};
...
Float transformLat(Coord point)
{
	Float lat = point.lat - 35;
	Float lng = point.lng - 105;
	Float ret = -100.0 + 2.0 * lng + 3.0 * lat + 0.2 * lat * lat + 0.1 * lat * lng + 0.2 * sqrt(abs(lng));
...
Float transformLng(Coord point)
{
	Float lat = point.lat - 35;
	Float lng = point.lng - 105;
	Float ret = 300.0 + lng + 2.0 * lat + 0.1 * lng * lng + 0.1 * lng * lat + 0.1 * sqrt(abs(lng));
	ret += (20.0 * sin(6.0 * lng * pi) + 20.0 * sin(2.0 * lng * pi)) * 2.0 / 3.0;
...
bool isOutOfChina(Coord point)
{
	return !(point.lng > 73.66 && point.lng < 135.05 && point.lat > 3.86 && point.lat < 53.55);
}

Coord converter(const Coord& pos, const double p[10])
{
	Float xTemp = p[0] + p[1] * abs(pos.lng);
	Float cC = abs(pos.lat) / p[9];
	Float yTemp = p[2];
	for (auto i = 1; i <= 6; ++i)
	{
	...
	xTemp *= (pos.lng < 0 ? -1 : 1);
	yTemp *= (pos.lat < 0 ? -1 : 1);
	return Coord(xTemp, yTemp);
}

Float getLoop(Float x, double low, double high)
{
	auto dif = high - low;
	while (x > high)
	{
...
Float getRange(Float x, double low, double high)
{
	return x > high ? high : (x < low ? low : x);
}

Coord GCJ02toBD09(Coord src)
{
	Float x = src.lng;
	Float y = src.lat;
	Float z = sqrt(x * x + y * y) + 0.00002 * sin(y * xPi);
	Float theta = atan2(y, x) + 0.000003 * cos(x * xPi);
	Float lng = z * cos(theta) + 0.0065, lat = z * sin(theta) + 0.006;
	return Coord(lng, lat);
}

GPS_Coordinate GPS_Coordinate::GCJ02toBD09(GPS_Coordinate src)
{
	return ::GCJ02toBD09(src).toGpsCoordinate();
}

Coord BD09toGCJ02(Coord src)
{
	Float x = src.lng - 0.0065;
	Float y = src.lat - 0.006;
	Float z = sqrt(x * x + y * y) - 0.00002 * sin(y * xPi);
	Float theta = atan2(y, x) - 0.000003 * cos(x * xPi);
	Float lng = z * cos(theta), lat = z * sin(theta);
	cout << std::setprecision(std::numeric_limits<Float>::digits10) << z << "\t" << theta << endl;
	return Coord(lng, lat);
}

GPS_Coordinate GPS_Coordinate::BD09toGCJ02(GPS_Coordinate src)
{
	return ::BD09toGCJ02(src).toGpsCoordinate();
}

Coord WGS84toGCJ02(Coord src)
{
	if (isOutOfChina(src))
	{
		return src;
	}
	Float lat = transformLat(src);
	Float lng = transformLng(src);
	Float radLat = deg2rad(src.lat);
...
{
	return ::WGS84toGCJ02(src).toGpsCoordinate();
}

Coord GCJ02toWGS84(Coord src)
{
	if (isOutOfChina(src))
	{
		return src;
	...
	src.lng -= lng;
	src.lat -= lat;
	cout << std::setprecision(std::numeric_limits<Float>::digits10) << lng << "\t" << lat << endl;
	return src;
}

GPS_Coordinate GPS_Coordinate::GCJ02toWGS84(GPS_Coordinate src)
{
	return ::GCJ02toWGS84(src).toGpsCoordinate();
}

GPS_Coordinate GPS_Coordinate::BD09toWGS84(GPS_Coordinate src)
{
	return ::GCJ02toWGS84(::BD09toGCJ02(src)).toGpsCoordinate();
}

GPS_Coordinate GPS_Coordinate::WGS84toBD09(GPS_Coordinate src)
{
	return ::GCJ02toBD09(::WGS84toGCJ02(src)).toGpsCoordinate();
}

Coord BD09toMC(Coord src)
{

	if (isOutOfChina(src))
		return Coord();
	auto lng = getLoop(src.lng, -180.0, 180.0);
	auto lat = getRange(src.lat, -74, 74);
...
{
	return ::BD09toMC(src).toGpsCoordinate();
}

Coord MCtoBD09(Coord src)
{
	Float x = abs(src.lng);
	Float y = abs(src.lat);
...
{
	return ::MCtoBD09(src).toGpsCoordinate();
}
